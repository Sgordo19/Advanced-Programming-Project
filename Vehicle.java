...
public class Vehicle {
	...
	Vehicle(int vehicle_id, int max_weight, int max_quantity, int current_weight, int current_quantity,
			boolean v_status) {
		this.vehicle_id = vehicle_id;
		this.max_weight = max_weight;
		this.max_quantity = max_quantity;
	...
	public void assignPackage(Package pkg) {
		...
		if (check_capacity(pkg.getWeight(), pkg.getQuantity())) {
			queue.Enqueue(pkg);
			System.out.println("Package " + pkg.getPackage_id() + " assigned to Vehicle " + vehicle_id);
		} else {
